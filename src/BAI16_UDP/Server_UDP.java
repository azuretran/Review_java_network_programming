    ...
    public static void main(String[] args) {
        ...
        try {
          ...
          //sort array
           sortDESC(array);
           for(i=0 ; i<length ; i++){
             if(array[i]== array[viTriMax2(array, length)]) resutString+=(" Phan tu thu "+i+" lon thu 2 trong mang a["+i+"]= "+array[i]+"\n");
                         if(array[i]== array[viTriMaxInt(array, length)]) resutString+=(" Phan tu thu "+i+" lon thu 1 trong mang a["+i+"]= "+array[i]+"\n");
                 sortDESC+=" "+array[i];     
            
         }
           
                   DatagramPacket number_insert= new DatagramPacket(BUFFER, BUFFER.length);
                ds.receive(number_insert); // Chờ nhận gói tin gởi đến
                   String number1_insert = new String(number_insert.getData(), 0, number_insert.getLength());
                System.out.println("Received: " + number1_insert);
                ...
                ds.send(outsending);
                // Tạo gói tin gởi chứa dữ liệu vừa nhận được
                DatagramPacket outsending1 = new DatagramPacket(sortDESC.getBytes(), sortDESC.length(),
                        number.getAddress(), number.getPort());             
                ds.send(outsending1);
                array = insert(array, Integer.parseInt(number1_insert));
           ...
           for(i=0 ; i<length+1 ; i++){
                ...
                // Tạo gói tin gởi chứa dữ liệu vừa nhận được
                DatagramPacket outsending11 = new DatagramPacket(sortDESC1.getBytes(), sortDESC1.length(),
                        number.getAddress(), number.getPort());             
                ds.send(outsending11);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
